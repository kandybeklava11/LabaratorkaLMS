...
public class Methoods implements GroupInterfaces,LessonInterfaces, StudentInterfaces {
    ...
    @Override
    public List<Lesson> getAllLessons(List<Lesson> lessons) {
    ...
    @Override
    public void GetLessonByName(List<Lesson> lessons) {
        System.out.println("Write the lesson's name:");
        String searchQuery = scanner.nextLine();
        List<Lesson> foundLessons = new ArrayList<>();

        for (Lesson lesson : lessons) {
            if (lesson.getLessonName().toLowerCase().contains(searchQuery.toLowerCase())) {
                foundLessons.add(lesson);
            }
        }

        if (foundLessons.isEmpty()) {
            System.out.println("No lessons found with the specified query.");
        } else {
            System.out.println("Search results:");
    ...
    @Override
    public void FindStudentByFirstName(List<Student> students) {
        System.out.println(" write student's first name");
        String foundStedent = scanner.nextLine();
        for (Student stud : students) {
            if (!stud.getStudentFirstName().equalsIgnoreCase(foundStedent)) {
                System.out.println(" Not found");
            } else {
